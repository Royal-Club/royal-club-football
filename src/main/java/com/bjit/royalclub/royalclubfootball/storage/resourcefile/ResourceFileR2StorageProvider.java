...
@Slf4j
public class ResourceFileR2StorageProvider implements ResourceFileStorageProvider {
    ...
    public ResourceFileR2StorageProvider(String endpoint, String bucket,
    ...
    public TeamLogoUploadResponse generateUploadUrl(String fileName, String contentType) {
        String key = "resource-" + UUID.randomUUID() + getSafeExtension(fileName);
    ...
    public InputStream load(String key) throws IOException {
        try {
            ...
            throw new IOException("Failed to load resource file", ex);
        }
    }
    ...
    private static String getSafeExtension(String originalFileName) {
        ...
        if (dotIndex < 0 || dotIndex == originalFileName.length() - 1) return ".bin";
